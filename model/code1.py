...
def getValue(z0,x0):
    ...
    # value = sum(p) / len(p)
    return x,u1,u2,w1,w2,p1,p2,q1,q2

#主函数
if __name__ == "__main__":
    x,u1, u2, w1, w2, p1, p2, q1, q2  =   getValue(59000,98358.9096531918)
    ...
    plt.plot(x, w1, color="blue", linewidth=1.0, linestyle="--",label='振子')
    plt.plot(x, u1, color="red", linewidth=1.0, linestyle="--",label='浮子')  # 将散点连在一起
    plt.legend(loc=0, prop=font, labelspacing=2, frameon=True)
    plt.xlabel('时间/s')
    plt.ylabel('位移/m')
    plt.savefig("位移曲线.svg", dpi=300, format="svg")
    plt.show()

    plt.rcParams['font.sans-serif'] = ['SimHei']
    plt.rcParams['axes.unicode_minus'] = False
    plt.plot(x, q1, color="blue", linewidth=1.0, linestyle="--", label='振子')
    plt.plot(x, p1, color="red", linewidth=1.0, linestyle="--",label='浮子')  # 将散点连在一起
    plt.legend(loc=0, prop=font, labelspacing=2, frameon=True)
    plt.xlabel('时间/s')
    plt.ylabel('角位移/m')
    plt.savefig("角位移曲线.svg", dpi=300, format="svg")
    plt.show()
    ...
    plt.xlabel('时间/s')
    plt.ylabel('位移差/m')
    plt.savefig("位移差曲线.svg", dpi=300, format="svg")
    plt.show()
    ...
    plt.xlabel('时间/s')
    plt.ylabel('角位移差/m')
    plt.savefig("角位移差.svg", dpi=300, format="svg")
    plt.show()

    aver = [x,u1,w1,p1,q1]

    with open("D:\Desktop\\运动数据.csv", "w", newline="") as csvfile:
