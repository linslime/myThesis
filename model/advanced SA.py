...
def MonteCarlo(n):
    ...
    bestValue = 0
    for i in range(n):
        c1 = random.random()*1000
        d1 = random.random()*1000
        value = getValue(c1,d1)
        # print(c1,d1,value)
        if value > bestValue:
            bestC1 = c1
            bestD1 = d1
            bestValue = value
    return bestC1,bestD1,bestValue

#主函数
if __name__ == "__main__":
    z0,x0,value = MonteCarlo(10)
    aver = []
    nexts = [15000,2000,100,10]
    turns = [50,100,20,20]
    sss = time.time()
    for n in range(len(nexts)):
        ...
        for i in range(turns[n]):
            [tempc, tempd] = newans(z0, x0, nexts[n], nexts[n])
            tempValue = getValue(tempc, tempd)
            # print(tempc,tempd, tempValue)
            if tempValue > value or random.random() < math.exp(-1 * (value - tempValue) / T):
                # aver.append([c1, d1, tempValue])
                print(tempc, tempd, tempValue,time.time() - sss)
                value = tempValue
                z0 = tempc
                x0 = tempd
            T = T * 0.9
